...
nS <- natanson_supplementary |> 
  group_by(Specimen) |> 
  summarise(n_ver = max(Column),
            max_d = log10(max(`Dorsal.Diameter.(mm)`)),
            max_t = log10(max(`Thickness.(mm)`)),
            fl_cm = log10(max(`FL.(cm)`))) |> 
  as.data.frame() |> 
  column_to_rownames(var = "Specimen")

pos_model_all <- glm(n_ver ~ fl_cm + max_d + max_t, family = "poisson",data = bind_rows(gws_natanson_log,nS))
