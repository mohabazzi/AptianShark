---
title: "Continuity of large-body size in the evolution of lamniform sharks"
...
## Docker Workflow and Github Action

```{r message=FALSE}
workflow <- rworkflows::use_workflow()
```

## Libraries

```{r message=FALSE, results='hide',warning=FALSE}
R.packages <- c("ozmaps ggpubr ggspatial dplyr openxlsx ggplot2 tidyr
                 purrr readxl kableExtra knitr gtsummary gt MASS broom 
                 smatr MASSTIMATE ggpmisc ggrepel smatr performance downloadthis
                 rempsyc car forcats RRPP caper geiger phangorn lmtest 
                 MuMIn phytools conflicted tibble irr nlme ggdist rworkflows")
...
```{r}
#' @title Averaging digitized measurements 
#' @description This function returns a dataframe obtained by applying the Reduce function. 
#' @return a dataframe.
...
numeric_details <- function(path = NULL, remove = NULL) {
  ...
  # Compute the average.
  numeric_details <- Reduce(f = "+",x = numeric_var)/length(numeric_var)
  if (!is.null(remove)) {
    numeric_details <- numeric_details |> select(-all_of(remove))
  }
...
alopias_pelagicus_liu <- 
  cbind(
    numeric_details(path = "data/Liu et al. 1999.xlsx",remove = "replicate"),
...
alopias_pelagicus_drew <- 
  cbind(
    numeric_details(path = "data/Drew et al. 2015.xlsx"),
...
alopias_superciliosus_data <- 
  cbind(
  numeric_details(path = "data/Liu et al. 1998.xlsx",remove = "replicate"),
...
alopias_vulpinus_data <- 
  cbind(
    numeric_details(path = "data/Cailliet et al. 1983.xlsx",remove = "replicate"),
...
carcharias_taurus_data <- 
  cbind(
    numeric_details(path = "data/Lucifora 2003.xlsx",remove = "replicate"),
    other_details(path = "data/Lucifora 2003.xlsx")
  ) |> 
  relocate(c("order","family","genus","species"),.before = 1) |> 
  select(!any_of(c("centrum_radius_mm","centrum_diameter_mm","specimen")))

# Carcharodon carcharias (reference: Cailliet et al. 1985, fig. 3, and Wintner and Cliff 1999, fig.)
carcharodon_carcharias_cailliet <- 
  cbind(
    numeric_details(path = "data/Cailliet et al. 1985.xlsx",remove = "replicate"),
...
carcharodon_carcharias_wintner <-
  cbind(
    numeric_details(path = "data/Wintner & Cliff 1999.xlsx"),
...
cetorhinus_maximus_data <- 
  cbind(
    numeric_details(path = "data/Natanson et al. 2008.xlsx",remove = "replicate"),
...
lamna_nasus_data <- 
  cbind(
    numeric_details(path = "data/Natanson et al. 2002.xlsx"),
...
lamna_dipturs_data <- 
  cbind(
    numeric_details(path = "data/Goldman & Musick 2006.xlsx",remove = "replicate"),
...
isurus_oxyrinchus_data <- 
  cbind(
    numeric_details(path = "data/Natanson et al. 2006.xlsx"),
...
pseudocarcharias_kamoharai_data <- 
  cbind(
    numeric_details(path = "data/Kindong et al. 2020.xlsx",remove = "replicate"),
...
In this section, **TL and BM** estimates are based on the white shark data from Wintner & Cliff (1999). Point estimates, 95% prediction and confidence intervals are computed for each specimen. 
...
mass_graph <- 
  ...
  annotate("text", x = 2350, y = 3.2, label = "upper~size~limit~of~italic(C.~carcharias)",
           parse = TRUE,col = "#B2182B",size = 3.5) +
...
```{r}
# Fit models.
common_slope <- lm.rrpp(log_tl ~ log_cd + species, data = lamniformes_data) # homogeneity of slopes test (additive model)
unqiue_slope <- lm.rrpp(log_tl ~ log_cd * species, data = lamniformes_data) # heterogeneity of slopes test (iteraction model)
...
model.comparison(common_slope,unqiue_slope,type = "logLik")
...
interSM1 <- lm(log_tl ~ log_cd, data = lamniformes_data)
# Lamniformes (excluding Alopidae)
interSM2 <- lm(log_tl ~ log_cd, data = lamniformes_data[!lamniformes_data$genus == "Alopias",])
# Lamniformes (excluding Alopidae and Cetorhinidae)
interSM3 <- lm(log_tl ~ log_cd, data = lamniformes_data[!lamniformes_data$genus %in% c("Alopias","Cetorhinus"),])
# Lamniformes (excluding all but Lamnidae)
interSM4 <- lm(log_tl ~ log_cd, data = lamniformes_data[lamniformes_data$family == "Lamnidae",])

# Compare LM2 versus LM4. How does TL change for one unit increase in CD?
...
# PPE.
ppe(true = 10^lamniformes_data$log_tl,pred = 10^fitted(interSM1))$mean
ppe(true = 10^lamniformes_data[!lamniformes_data$genus == "Alopias",]$log_tl,
    pred = 10^fitted(interSM2))$mean
ppe(true = 10^lamniformes_data[!lamniformes_data$genus %in% c("Alopias","Cetorhinus"),]$log_tl,
    pred = 10^fitted(interSM3))$mean
ppe(true = 10^lamniformes_data[lamniformes_data$family == "Lamnidae",]$log_tl,
    pred = 10^fitted(interSM4))$mean
```
...
The performance of each species-level model is evaluated using the **Mean Absolute Error (MAE)**. This MAE is a measure of **goodness-of-fit** and represents the average absolute difference between the predicted (fitted) values and the actual target values (in this context TL).
...
lapply(maeR, function(x) { Metrics::mae(x$log_tl, x$fit) }) |> 
  ...
  mutate("mae" = round(mae,digits = 2)) # Similar to MAE results above.
...
```{r fig.cap="Relationship between centrum diameter and total length in C. carcharias. Data is based on records from Wintner & Cliff (1999) and Cailliet et al. 1985. The estimated TL along with 95% prediction intervals of the cardabiodontid specimens are added to the graph", warning=FALSE}
...
lamnidae_data |> 
  ...
  labs(title = NULL,subtitle = NULL,
       y = "Count",
       x = "Predicted total length (log 10)") +
  theme_bw() +
  theme(axis.title = element_text(face = "bold",size = 10),
        axis.text = element_text(size = 8),
        axis.text.y = element_text(angle = 90,hjust = 0.5,vjust = 0.5),
...
<!--Using maximum size; weighted approach or within-group fit -->

```{r}
# Genetic Data Only Tree.
sgtree <- phytools::readNexus(file = "phylogeny/500treePLtrees.nex")

# MCC Tree.
mcc_sgTree <- phangorn::maxCladeCred(x = sgtree,rooted = T)

# Compute trait averages per species.
grp_average <- 
  lamniformes_data |>
  group_by(species) |>
  summarise(ave_tl = mean(log_tl),ave_cd = mean(log_cd)) |> 
  as.data.frame()

grp_max <- 
  lamniformes_data |>
  group_by(species) |>
  summarise(ave_tl = max(log_tl),ave_cd = max(log_cd)) |> 
  as.data.frame()

rownames(grp_average) <- grp_average$species

grp_average_clean <- data.frame(CD = setNames(grp_average$ave_cd,gsub(" ", "_", grp_average$species)),
                                TL = setNames(grp_average$ave_tl,gsub(" ", "_", grp_average$species)),
                                species = gsub(" ", "_", grp_average$species))

# Drop tips.
lamn_tree <- geiger::treedata(phy = mcc_sgTree,data = grp_average_clean, warnings = FALSE)$phy
lamn_tree |> 
  plot(); axisPhylo(1) 
...
grp_pgls$Cov <- vcv.phylo(lamn_tree)

pgls_shark <- lm.rrpp(f1 = TL ~ CD,data = grp_pgls, Cov = grp_pgls$Cov,  print.progress = FALSE)
# pgls_shark |> anova()
nonPhylo_shark <- lm.rrpp(f1 = TL ~ CD,data = grp_pgls,  print.progress = FALSE)

model.comparison(pgls_shark,nonPhylo_shark,type = "logLik")
...
cbind(
  ...
  knitr::kable(caption = "Table X. Phylogenetic signal in CD and TL") |>
  kableExtra::kable_styling()
```
...
# Body size ranges.
size_trait <- 
  openxlsx::read.xlsx(xlsxFile = "lamniformes-body-ranges.xlsx") |> 
...
# Four specimens with size, vertebrae count, and centrum diameter.
gws_natanson <-
...
10^predict(object = interSM4,newdata = stem_species,interval = "prediction") |> 
  data.frame() |> 
  add_column("specimen" = mesozoic_data$label) |> 
  mutate(CD= mesozoic_data$centrum_diameter/10) |> 
  add_row(fit = 718.7564, lwr = 608.9134, upr = 848.4143,specimen = "NTM P 922-32", CD = 12.6) |> 
  # Order by age
  ...
  geom_rect(xmin = 0.7, xmax = 1.3, ymin = 600, ymax = 870,lty = 2,alpha = 0, color = "black") +
...
10^predict(object = interSM4,newdata = data.frame(log_cd = cooper_megladon$log_cd),interval = "prediction") |> 
  as.data.frame() |> 
  ggplot(aes(x = 1:141,y = fit)) +
...
macro_ecology <- 
  lamniformes_data |> 
  mutate(ecology = case_when(species == "Cetorhinus maximus" ~ "Microphagous", T ~ "Macrophagous")) |> 
  mutate(ecology = as.factor(ecology)) |> 
  mutate(lamellae_type = case_when(species == "Cetorhinus maximus" ~ "Thick concentric lamellae", T ~ "Thin concentric lamellae")) |> 
  mutate(lamellae_type = as.factor(lamellae_type)) |> 
  select(log_cd,log_tl,ecology,lamellae_type)

# Predict (use Lamnidae Tl estimates)
fossil_dataset <- data.frame(log_tl = log10(IS.IIII$fit), log_cd = aptian_shark$log_cd,lamellae_type = "Thin concentric lamellae")
fossil_dataset$lamellae_type <- factor(fossil_dataset$lamellae_type, levels = levels(macro_ecology$lamellae_type))

rf_model <- randomForest(
  ecology ~ log_cd + log_tl + lamellae_type,
  data = macro_ecology,
  ntree = 1000,
  mtry = 2,
  importance = TRUE
...
predict(rf_model, newdata = fossil_dataset,type = "prob") |> 
  as.data.frame() |> 
  mutate(specimen = c("NTM P 922-33","NTM P 922-32","NTM P 9382", "NTM P 922-37","NTM P 922-29")) |> 
  tidyr::pivot_longer(cols = c("Macrophagous","Microphagous"),names_to = "Ecology",values_to = "Predictions") |> 
  ggplot(mapping = aes(x = "", y = Predictions, fill = Ecology)) +
  geom_bar(stat = "identity",position = "fill") +
  labs(x = "%", y = NULL) +
  scale_fill_manual(values = c("#2D716F","#2E2D29")) +
